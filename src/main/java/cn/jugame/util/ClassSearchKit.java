...
import cn.jugame.util.reflect.Reflect;

public class ClassSearchKit {

	private static final Logger LOG = LoggerFactory.getLogger(ClassSearchKit.class);
	...
	private static List<String> findFiles(String baseDirName, String targetFileName) {
		...
		try {
			baseURLs = ClassSearchKit.class.getClassLoader()
					.getResources(File.separator + baseDirName.replaceAll("\\.", "/"));
		} catch (IOException e) {
			e.printStackTrace();
		}
		URL baseURL = null;
